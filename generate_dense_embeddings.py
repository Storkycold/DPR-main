...
def gen_ctx_vectors(
    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        ctx_ids_batch = move_to_device(torch.stack(
            batch_token_tensors, dim=0), cfg.device)
        ctx_seg_batch = move_to_device(
            torch.zeros_like(ctx_ids_batch), cfg.device)
        ...
        if extra_info:
            results.extend([(ctx_ids[i], out[i].view(-1).numpy(),
                           *extra_info[i]) for i in range(out.size(0))])
        else:
            results.extend([(ctx_ids[i], out[i].view(-1).numpy())
...
def main(cfg: DictConfig):
    ...
    all_passages_dict = {}
    with open(ctx_src.file_path, "r", encoding="utf-8") as tsvfile:
